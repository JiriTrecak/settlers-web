...
**Exit** (left of the minimap) asks before tearing the match down. **Save replay** snapshots the log at this tick (live matches only). Escape deselects (ghost → claim → hut); it does not leave. Speed under the minimap. Replay watch: claim hidden; timeline is bottom-middle.
