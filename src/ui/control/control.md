...
Session mounts `Minimap` into `minimapHost` and pushes `CommandPage` / `SelectionView`. Clicks emit `onCommand(id)` only — no World, no place-tool. Keys are not handled here; `CommandBoard.key` matches the painted page.
