...
`Minimap.onLookAt` → `Camera.lookAt`. `SpeedControl.onSpeed` multiplies wall-clock dt into the accumulator (quantum stays 25ms). Space → `Renderer.fitCamera`. Escape → `onLeave` (lobby). Each frame pushes debug counts into the HUD (F3). `setShowPaths` is the overlay toggle — stored on the session so it survives if the renderer is not up yet.
