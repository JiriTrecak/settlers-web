...
Match recording for debug (and later, lockstep). File is seed + map id + action log + duration + checksum. `ReplayStore` shelves it in localStorage when `World.outcome` first fires.
