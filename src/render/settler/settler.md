...
Clips keyed by profession: `settlers/roman/{type}/{walk,idle,action1,action2,bend}/{none,trunk,plank,tree,...}/{dir}`. Chop/saw/harvest loop `action1` once per second for the whole work window. Farmer **plant** (`plantCrop`) loops `action2`. Pickup/drop/deliver play `bend` once. Missing clip → that profession's idle, then bearer.
