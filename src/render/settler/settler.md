...
`SettlerLayer.draw` skips `inside` units (in the hut). Otherwise lerps `from`→`pos` with `moveProgress` plus the session's leftover tick fraction. z = `isoDepth` (same container as props, so stones can cover the unit). Missing catalog → yellow dot.
