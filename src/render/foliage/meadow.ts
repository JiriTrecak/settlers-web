...
import { BufferGeometry, Float32BufferAttribute, InstancedMesh, MeshLambertMaterial, DoubleSide, Color, Object3D, type Scene, type IUniform, TextureLoader, SRGBColorSpace, LinearSRGBColorSpace } from 'three';
...
export class Meadow {
  ...
  private material(grass:boolean,broad=false):MeshLambertMaterial {
    const m=new MeshLambertMaterial({side:DoubleSide, map:grass&&!broad?this.tuft:null, alphaTest:grass&&!broad?.45:0,alphaToCoverage:grass&&!broad});
    m.onBeforeCompile=s=>{
  ...
  rebuild(field:HeightField,landscape:Landscape):void {
    ...
    for(const patch of landscape.cover){
      ...
      const hue=palette==='ochre'?.085:palette==='sage'?.17:palette==='straw'?.10:season==='autumn'?.13:season==='spring'?.25:.27;
      const saturation=palette==='ochre'?.5:palette==='sage'?.23:.58;
      ...
      for(let i=0;i<n && poses.length<90000;i++){
        ...
        const c=flower?new Color([0xfff4d0,0xfff4d0,0xfff4d0,0xf3a2eb,0xad7cf0,0x7faaf4,0xffe36f,0xfff4d0,0xffe36f,0xe983eb,0x8674ed,0xffdf63][Math.floor(rand()*12)]!):new Color().setHSL(hue+rand()*.02,saturation+rand()*.1,(palette==='sage'?.43:palette==='ochre'?.5:.47)+rand()*.09,palette==='meadow'&&season!=='autumn'?SRGBColorSpace:LinearSRGBColorSpace);
        const pose={x,y:y-.035,z,s:flower?.6+rand()*.5:(1.1+rand()*.7)*(patch.grassScale??1),r:rand()*6.28,flower,broad:rand()<(patch.broadRatio??.55),c};
    ...
    for(let idx=0;idx<3;idx++){
      ...
      const mesh=new InstancedMesh(flower?this.flowers:idx===1?this.broadGrass:this.grass,this.materials[idx]!,list.length);
  ...
  destroy():void{for(const m of this.meshes){this.scene.remove(m);m.dispose();}this.tuft.dispose();this.broadGrass.dispose();this.grass.dispose();this.flowers.dispose();this.materials.forEach(m=>m.dispose());}
}
