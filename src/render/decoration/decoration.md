...
- `decorationSheets.ts` — loads `/graphics/catalog.json` groups `props/tree-1..7`, `props/tree-fall-1..4`, `props/stone`, `props/waves`, `props/stack-{material}`, `props/found-{sign}` (falls back to `props/site-sign`), `props/border`. Missing catalog → layer no-ops, no crash.
