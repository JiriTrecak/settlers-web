...
export class AtmospherePass {
 ...
 render(gl:WebGLRenderer,scene:Scene,camera:Camera,frame:AtmosphereFrame,measure:(label:string,draw:()=>void)=>void=(_,draw)=>draw()):void {
  ...
  if(!settings?.enabled||quality==='off'){perf.value('Atmosphere','Off');perf.sample('GPU atmosphere',0);perf.sample('Atmosphere submit (CPU)',0);measure('GPU scene',()=>gl.render(scene,camera));return;}
  ...
  if(this.fogTarget.width!==width||this.fogTarget.height!==height){this.fogTarget.setSize(width,height);this.filteredTarget.setSize(width,height);}
  const filtered=gl.shadowMap.type===PCFShadowMap,key=`${filtered}/${steps}`;
  if(key!==this.shaderKey){this.shaderKey=key;this.march.defines={STEPS:steps,...(filtered?{FILTERED_SHADOW:1}:{})};this.march.needsUpdate=true;}
  ...
  try{
   ...
   gl.shadowMap.autoUpdate=false;this.quad.material=this.march;gl.setRenderTarget(this.fogTarget);gl.render(this.quadScene,this.camera);
