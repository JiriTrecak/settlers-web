...
export function applyPlayerMaterial(
  ...
  const flag =
    material.name === "UTC Team color" || material.name === "Ant faction red";
  const shell =
    /^Ant carapace roof \d+$/.test(material.name) ||
    material.name === "Ant rust chitin" ||
    material.name === "Ant chitin planes";
  if (!flag && !shell) return;
  const original = (material.userData.authoredPlayerColor ??=
  ...
  const source = new Color()
    .fromArray(original)
    .getHSL({ h: 0, s: 0, l: 0 }, SRGBColorSpace);
