...
The active warrior is the approved Tripo model, authored in `art/sources/characters/ant-warrior-tripo/` and published to `assets/library/asset.models.units.ants-warrior/geometry.glb` by `scripts/assets/publish-ant-warrior.ts`. It has 4,905 triangles, seven texture maps, `TC_TeamColor` clothing/leaves, and idle/walk/run/charge/attack/hit/death states (`charge` uses run). Its attack contact is 35/71 of the clip, synchronized to the simulation impact tick. It has no facial rig or carry pose. Hand/blade socket names retain the shared engine contract. The family workflow below describes the other ant roles and legacy shared source.

Tune **`rules.unitScale` in `content/game.json`** (currently **1.7**) and reload/start a new match. This single multiplier controls every unit's model/attachments/indicators, walk/run/charge speed, collision radius, terrain and bridge clearance, formation spacing, and melee reach. Source meshes and authored stats stay at scale 1; the content registry resolves gameplay stats once. Buildings, portraits, attack cadence/damage, vision and ranged weapon distances do not scale. Movement clips retain their cadence because their enlarged stride and world speed grow together. Pathfinding and physical sweeps agree on body size, including routes around narrow gaps and bridge headroom. The setting participates in the match fingerprint, so saves/multiplayer cannot silently mix scales. Supported range is 0.25–4; very large units naturally cannot fit existing narrow spaces.

For quick experiments, open `/combat-lab.html`, change **Unit scale**, and press **Reset scenario**, then **Engage**. The lab constructs both the simulation and renderer from the same temporary rules. This does not change the saved default; put the preferred value in `content/game.json` afterward.
