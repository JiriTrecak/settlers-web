...
`library.ts` discovers project `.utcmap` files in the campaign, skirmish and showcase folders (retired tutorial copies are excluded) and current-format locally saved maps. Both Editor and Singleplayer use it. New supplies required starts. No generated fallback map, resource inference or old-format loading remains. See [authoring](../../../docs/declarations/README.md).
...
Worldroot Hollow (256 × 256), Terrain Proving Ground (256 × 256), and Four Crowns (512 × 512, four players) are shipped battlefields. See `tacticalTerrain.md` for height rules and authoring. Worldroot Hollow’s saved document is `assets/maps/skirmish/worldroot-hollow.utcmap`; the reproducible scenery pass is `scripts/maps/rebuild-canopy-skirmish.ts`. Two guarded Root sites, ten camps, clustered woodland, four ponds and branching streams surround two home clearings.
...
The Vanguard campaign contains Prologue, Lantern Hearth and The Bitter Root. Mission metadata excludes these maps from skirmish. Canopy Clearing is the two-player art-direction fixture.
