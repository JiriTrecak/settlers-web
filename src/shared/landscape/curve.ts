...
export type CoverPatch = { exclusions?: CoverExclusion[]; x: number; z: number; radius: number; density: number; seed: number; flowers: number; grassScale?: number; broadRatio?: number; palette?: 'meadow' | 'straw' | 'ochre' | 'sage' };
export type EnvironmentState = { preset?: string; hour: number; season: 'spring' | 'summer' | 'autumn'; playing: boolean };
