...
export type DaytimeSample={phase:DaytimeId;from:DaytimeId;to:DaytimeId;blend:number;look:DaytimeLook;lutWeights:{path:string;weight:number}[]};
...
const hold=(id:DaytimeId):DaytimeSample=>({phase:id,from:id,to:id,blend:0,look:DAYTIME_LOOKS[id],lutWeights:[{path:DAYTIME_LOOKS[id].textureColorLUT,weight:1}]});
const HOLDS={day:hold('day'),night:hold('night'),day_to_night:hold('day_to_night'),night_to_day:hold('night_to_day')};
export function sampleDaytime(hour:number):DaytimeSample {
 ...
 const a=DAYTIME_LOOKS[from],b=DAYTIME_LOOKS[to],num=(x:number,y:number)=>lerp(x,y,t);
 return {phase:middle,from,to,blend:t,lutWeights:[{path:a.textureColorLUT,weight:1-t},{path:b.textureColorLUT,weight:t}],look:{
