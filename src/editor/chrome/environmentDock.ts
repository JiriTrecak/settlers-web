...
export class EnvironmentDock {
  ...
  constructor(host:HTMLElement,private readonly editor:WorldEditor,close:()=>void){
    ...
    const timing=document.createElement('p');timing.className='text-xs text-canopy/50';timing.textContent='240s full cycle. Light controls tune daytime; the clock still drives sun movement. Editing light pauses the preview.';time.append(timing);
