...
- `dumpedMap.ts` — JSON schema from `original_conv` (`heights`, `landscape` as our type names, `trees` with `sheet`, `stones` with `capacity`, `starts` from the player-info block).
