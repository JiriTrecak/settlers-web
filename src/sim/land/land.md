...
Same-player overlap is a union. `release` drops one disk and replays the rest — overlapping land stays, the rest goes unowned. Pioneer tiles are not disks, so they vanish only if something else overwrites them. Once any disk exists, `canPlace` requires the plot on that player's land, outdoor work goes through `acceptWork` / plant search, and settlers with `needsPlayersGround` (default true) path / flock on their own ground. Rim posts: owned, not water, hex neighbor a different owner. Render hides them at sight ≤50.
