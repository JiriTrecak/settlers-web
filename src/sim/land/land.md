...
Per-tile owner (`-1` unowned) plus a tower-count. Garrisoned occupying buildings (HQ and extra T1 towers) and debug claim stamp a `TOWER_RADIUS` disk via `forEachCircleTile`, clipped to the map. Pioneers `claim` one tile if `towerCount == 0` — they do not steal tower-covered ground and do not bump the counter.
