...
Lumberjack: rest **inside** the hut (`restMs`), fell the nearest unclaimed tree in `workRadius` (skip trees another lumberjack already has a `chop` on, skip saplings, skip if the SE stand tile is blocked). Stands **SE of the tree, faces nw**, carry the trunk, dump on the offer stack (capacity 8), walk home, enter, rest, repeat. Full stack or no trees → stay inside.
