...
export function dropPosition(c:GameContext,origin:Point,except?:number):Point {
 ...
 for(let radius=0;radius<=6;radius++){
  for(let dy=-radius;dy<=radius;dy++)for(let dx=-radius;dx<=radius;dx++){
   ...
   if(c.spatial.free(p,except)&&c.spatial.clearSegment(fixed(origin),fixed(p))&&!items.some(e=>e.surface===p.surface&&(e.x-p.x)**2+(e.y-p.y)**2<4))return p;
  }
 }
 // A completely crowded death site must still preserve every earned item.
 return {x:origin.x,y:origin.y,...(origin.surface?{surface:origin.surface}:{})};
}
