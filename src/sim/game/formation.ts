 ...
 */
export function formationDestinations(members:readonly Member[], destination:Point, size:number, walkable:(p:Point)=>boolean,straight:(from:Point,to:Point)=>boolean=()=>true,spacing=1):Map<number,Point>{
 const result=new Map<number,Point>();if(!members.length)return result;
 const separated=(a:Point,b:Point)=>(a.x-b.x)**2+(a.y-b.y)**2>=spacing*spacing-1e-9;
 ...
 if(members.length>1){
  ...
  const maxSpan=Math.ceil(Math.sqrt(actors.length))*2*Math.max(1,spacing);
  if(width*height<=actors.length*2*Math.max(1,spacing*spacing)&&width<=maxSpan&&height<=maxSpan){
   ...
   if(new Set(translated.map(p=>p.y*size+p.x)).size===actors.length&&translated.every((p,i)=>translated.slice(0,i).every(q=>separated(p,q)))&&translated.every((p,i)=>
 ...
 const slots:Point[]=[];
 const searchRadius=Math.min(size-1,Math.max(12,Math.ceil(Math.sqrt(members.length))*3*Math.max(1,spacing)));
 for(let radius=0;slots.length<members.length && radius<=searchRadius;radius++){
  ...
  for(const candidate of ring){
   ...
   if(slots.every(p=>separated(p,candidate)))slots.push(candidate);
  }
 }
 const actors=[...members].sort((a,b)=>a.x-b.x||a.y-b.y||a.id-b.id);
