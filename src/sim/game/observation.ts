...
export type EntityView = {
  ...
  control?: {
    ...
    job: {type: import('./state').Job['type']; target:number; source:number|null; item:string|null; phase:import('./state').Job['phase']} | null;
...
const observedDeathSchema=z.object({id:z.int().positive(),definition:z.string(),x:z.number(),y:z.number(),tick:z.int().nonnegative()}).strict();
...
const memoryEntity = z
  .object({
    ...
    stats: z.object({level:z.number().int().positive(),maxHp:z.number().int().nonnegative(),damage:z.number().int().nonnegative(),armor:z.number().int().nonnegative()}).strict().optional(),
    appearance: z
...
export const knowledgeSchema = z.array(
  z
    .object({
      ...
      territory: z.array(z.number().int().min(-1).max(7)).max(262144),
      borders: z.array(z.number().int().min(0).max(4)).max(262144),
...
export class Observation {
  ...
  private readonly territoryVersions = new Map<Owner, number>();
  ...
  recordDeath(e: Entity) {
    ...
    for(const m of this.memories)if(e.owner===m.owner||this.previouslyVisible(m.owner,e)){
      m.observedDeaths.push({id:e.id,definition:e.definition,x:e.x,y:e.y,tick:this.c.state.tick});
  ...
  constructor(
    ...
    this.memories = owners.map((owner) => ({
      ...
      visibleCells:[],
      territory: new Int16Array(this.c.spatial.size**2).fill(-1),
      borders: new Uint8Array(this.c.spatial.size**2),
      entities: new Map(),
      observedDeaths:[],
    }));
  }
  ...
  private describe(e: Entity, privateData: boolean, observer?:Owner): EntityView {
    const result: EntityView = {
      ...
      ...(privateData&&e.revival?{revival:structuredClone(e.revival)}:{}),
      ...(privateData && e.progression ? {progression:{...e.progression}} : {}),
      ...(privateData && e.spellcasting ? {spellcasting:structuredClone(e.spellcasting)} : {}),
    ...
    if (e.unit)
      result.unit = {
        ...
        ...(e.spellcasting?.pending?{casting:{ability:e.spellcasting.pending.ability,resolveTick:e.spellcasting.pending.resolveTick}}:{}),
        ...
        ...(e.unit.shot && (!observer || e.unit.shot.viewers.includes(observer)) && this.c.state.tick-e.unit.shot.tick<24
          ? {shot:{tick:e.unit.shot.tick,x:e.unit.shot.x,y:e.unit.shot.y}} : {}),
      };
    if (e.unit && result.unit && !result.unit.moving && !result.unit.contained &&
      (e.unit.goal === null || atPoint(e, {x: e.unit.goal % this.c.spatial.size, y: Math.floor(e.unit.goal / this.c.spatial.size)}))) {
      ...
      const creation = job?.type === "construct" || job?.type === "repair"
        ...
        : job?.type === "harvest" && job.phase !== "return" && job.item
          ? this.c.registry.get(job.item).creation : undefined;
    ...
    if (privateData) {
      ...
      if(e.unit){
        ...
        result.control={order:structuredClone(e.unit.order),employment:e.unit.employment,
          job:j?{type:j.type,target:j.target,source:j.source,item:j.item,phase:j.phase}:null,
          pendingMove:e.unit.pendingMove?{...e.unit.pendingMove}:null,releasing:!!e.unit.release,stunned:isStunned(e,this.c.registry)};
      }
  ...
  update() {
    ...
    for (const m of this.memories) {
      m.observedDeaths=m.observedDeaths.filter(d=>this.c.state.tick-d.tick<=400);
      const sensors = this.c.live().filter(e => e.owner === m.owner && !e.unit?.contained && !e.unit?.release);
      const signature = sensors.map(e => `${e.id}:${e.x}:${e.y}:${this.c.def(e).vision ?? 0}`).join(';');
      const visionChanged = this.sensorSignatures.get(m.owner) !== signature;
      const territoryChanged = this.territoryVersions.get(m.owner) !== this.c.spatial.territoryRevision;
      this.sensorSignatures.set(m.owner, signature);
      this.territoryVersions.set(m.owner, this.c.spatial.territoryRevision);
      if (visionChanged) {
        ...
        for (const sensor of sensors) {
          ...
          for (let dy = -r; dy <= r; dy++) {
            ...
            const lo=y*this.c.spatial.size+Math.max(0,sensor.x-span),hi=y*this.c.spatial.size+Math.min(this.c.spatial.size-1,sensor.x+span);
            for(let i=lo;i<=hi;i++)if(m.cells[i]!==2){m.cells[i]=2;m.visibleCells.push(i);}
          }
        }
      }
  ...
  view(owner?: Owner): SettlementView {
    ...
    const result: SettlementView = {
      ...
      fallenHeroes:this.c.state.entities.filter(e=>e.fallen&&(!owner||e.owner===owner)).map(e=>this.describe(e,true,owner)),
      visuals:this.c.state.visuals.filter(v=>!owner || v.viewers.includes(owner)).map(v=>structuredClone(v)),
  ...
  checksum() {
    ...
    return simulationHash(this.memories.map(({visibleCells: _index, ...memory}) => memory));
  }
  snapshot() {
  ...
  validateSnapshot(raw: unknown) {
    ...
    for (const row of rows) {
      if([row.cells,row.territory,row.borders].some(a=>a.length!==this.c.spatial.size**2))throw new Error("Invalid knowledge dimensions");
  ...
  restore(raw: unknown) {
    ...
    this.territoryVersions.clear();
    this.deathCues.length = 0;
    for (const m of this.memories) {
      const row = rows.find((r) => r.owner === m.owner)!;
      m.cells = Uint8Array.from(row.cells);
      m.visibleCells=[];for(let i=0;i<m.cells.length;i++)if(m.cells[i]===2)m.visibleCells.push(i);
