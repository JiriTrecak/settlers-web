...
Add a hut → new `buildings/{kind}.ts` + one line in `buildings/index.ts`. Same for professions. Mines set `flatten: false` and `mine` (iron / gold). Miner `tool: "pick"` — occupy equips that pile first. Farm sets `workCenter`. Farmer `tool: "scythe"`, fisherman `tool: "fishingrod"`, slaughterer `tool: "axe"`.
