...
type Memory = {
  owner: number; cells: Uint8Array; territory: Int16Array;
  buildings: Map<number, Building>; resources: Map<number, ResourceNode>;
};
export type VisionSnapshot = { revision: number; players: {
  owner: number; cells: number[]; territory: number[]; buildings: Building[]; resources: ResourceNode[];
}[] };
const copyBuilding=(b:Building):Building=>({...b,escrow:{...b.escrow},delivered:{...b.delivered},inventory:{...b.inventory}});
...
export class Visibility {
  ...
  constructor(owners: readonly number[]) {
    this.players=[...owners].sort((a,b)=>a-b).map(owner=>({owner,cells:new Uint8Array(65536),territory:new Int16Array(65536).fill(-1),buildings:new Map(),resources:new Map()}));
  }
  ...
  update(buildings:readonly Building[],workers:readonly Worker[],resources:readonly ResourceNode[],territory:Int16Array) {
    ...
    for(const p of this.players) {
      p.cells=p.cells.slice();p.territory=p.territory.slice();
      ...
      for(let i=0;i<65536;i++)if(p.cells[i]===2)p.territory[i]=territory[i]!;
    }
  }
  ...
  snapshot():VisionSnapshot {
    return {revision:this.revision,players:this.players.map(p=>({owner:p.owner,cells:Array.from(p.cells),territory:Array.from(p.territory),buildings:[...p.buildings.values()].map(copyBuilding),resources:[...p.resources.values()].map(n=>({...n}))}))};
  }
  restore(snapshot:VisionSnapshot) {
    ...
    for(const p of this.players){const s=snapshot.players.find(s=>s.owner===p.owner)!;p.cells.set(s.cells);p.territory.set(s.territory);p.buildings=new Map(s.buildings.map(b=>[b.id,copyBuilding(b)]));p.resources=new Map(s.resources.map(n=>[n.id,{...n}]));}
  }
  checksum():number {
    let h=2166136261;const mix=(n:number)=>{h=Math.imul(h^n,16777619);};
    mix(this.revision);
    for(const p of this.players){mix(p.owner);for(let i=0;i<65536;i++){mix(p.cells[i]!);mix(p.territory[i]!);}const text=JSON.stringify([[...p.buildings.values()],[...p.resources.values()]]);for(let i=0;i<text.length;i++)mix(text.charCodeAt(i));}
    return h>>>0;
  }
}
