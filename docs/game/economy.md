...
Amber Mines declare `gatheringUnitCollision: false`. Active gather orders and committed mine-return jobs pass through units in both path planning and motion execution. Other units can pass through these miners too. Pending mine orders do not grant this early; static terrain, buildings and resource footprints remain solid. Trees use normal collision. See [unit orders](../declarations/unit-orders.md) for queued delivery and construction semantics.

`item.amber`, `item.wood`, and `item.root` are currencies. Workers harvest a finite amount from a targeted source, carry a visible load, and deposit it at an owned completed drop-off (Mound for Amber/Wood; Rootworks exclusively for Root). Money becomes available only on delivery. A full trip carries **10 amber or 10 wood**. Amber gathers ten per 2.5-second work cycle. Trees have **10 chopping HP** and hold **10 wood**: each one-second axe cycle removes one HP at contact, and the tenth blow fells the tree. The worker waits for the 1.8-second fall before carrying the load home. Travel adds time. A nearly exhausted source can yield a smaller final load; resources are never invented to fill it. A tree requires room for its whole remaining load before work starts. These values are shared across gameplay, AI and editor-loaded maps. There are no loose currency entities, ground refunds, sawmills, lumberjack workplaces, stone deposits or planks. Hero equipment drops remain ground items and are independent of the economy.

The neutral `building.neutral.amber-mine` has a five-cell footprint, HP, a resource yield, an entrance, a three-cell construction clearance and ten gathering slots. It is selectable and targetable; it does not need to be captured or built. A gathering assignment occupies a slot throughout approach, harvesting and return to the Mound. Assignment is checked sequentially for group orders, so simultaneous commands cannot exceed capacity. Capacity is shared by everyone using that mine. Retargeting a carrier reserves the new mine immediately while retaining its previous slot until the old load is delivered; group orders cannot overbook during that handoff. Inspection shows remaining amber and occupancy; remembered mines do not expose live changes through fog.

Workers can right-click actual tree canopies. A tree is reserved by one worker at a time; other workers choose nearby trees. Interrupted chopping retains the tree’s remaining HP. The final contact assigns its wood to that worker and removes blocking occupancy, but does not deposit money remotely. The tree falls away from the worker, then sinks at full scale over six seconds. There are no loose logs or shrinking trees. A timber assignment can continue to another nearby tree of the same definition. Depleted mines do not silently redirect workers to a different mine. Foresters restore eligible exhausted tree sites using their native planting process; sites become blocking only when mature and clear of units.
...
The HUD presents the military reserve as `available / (available + max(0, capacity - workers))`. The second number projects the free reserve after empty slots replenish, assuming current assignments remain. Total workforce stays available to simulation and observer statistics. This is a presentation of the existing population rules, not a separate military capacity. After capacity loss, the denominator cannot fall below already available workers.
...
Every queued recruit is paid for immediately, including entries waiting behind another unit. An unaffordable request is rejected without a partial charge. Successful training consumes the reserved bill and changes the same worker entity into the soldier. A blocked exit retains the worker and its money. Cancelling releases the worker (or waits for a free release cell), removes only that stable queue entry and refunds its exact amber/wood bill. The next recruit is already paid for and cannot hide that refund with a new charge. Recruitment creates room in the worker pool only when conversion succeeds.
