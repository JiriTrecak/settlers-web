...
Chrome: name + file dock, left tools (stamp / brush / clean / sculpt / catalogue / grid / Gamecam). Brush paints a red mask; Apply scatters a weighted asset set. Water-type assets only land on wet cells and sit on the sea. Named presets persist. Clean wipes stamps in a disc (Objects now). Sculpt Live raises / lowers; Water paints a basin and Apply cuts it under the sea. Shift erases the brush mask or lowers sculpt; Shift+wheel size; Ctrl+wheel density. Catalogue modal to pick and create assets. Stamp with no selection opens the modal; Use / double-click arms the stamp tool. Camera orbits (Alt-LMB / MMB / RMB) unless Gamecam is on — then WC3 perspective, fixed zoom, pan to half a block past the red. Play is the same Gamecam.
