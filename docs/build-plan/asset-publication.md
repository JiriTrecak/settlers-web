...
The runtime manifest and direct renderer/UI art references now use canonical publication. Old source directories, old record archives, map documents, and several one-off authoring scripts still exist. They require audited retirement or conversion; do not run an old model-generation script and assume it publishes into the new library. Specialized capabilities and material metadata extraction are still in progress. This publication milestone does not complete the broader authoring goal.
