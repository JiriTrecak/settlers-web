...
const routes:Record<string,string>={library:'/library',jobs:'/jobs',create:'/jobs',assign:'/assignments',targets:'/assignments',reference:'/references'};
let body=file?JSON.parse(await readFile(file,'utf8')):undefined;
if(operation==='reference'&&body?.file){body.data=(await readFile(body.file)).toString('base64');delete body.file;}
...
if(!route)throw Error('Usage: assets:studio library|jobs|reference|create|process|approve|publish|cancel [request.json]');
