...
export async function recoverTransactions(root:string){return withWorkspaceWriteLock(root,async()=>{for(const f of await filesIn(path.join(root,'.asset-work/transactions')))if(f.endsWith('/journal.json')){const j=await json<{state:string;entries:Entry[]}>(f);if(j.state==='pending'){for(const entry of [...j.entries].reverse()){const file=await within(root,entry.path);if(entry.backup)await atomic(file,await readFile(entry.backup));else await rm(file,{force:true});}await saveJson(f,{...j,state:'rolled-back'});}}await rm(path.join(root,'.asset-work/publishing'),{force:true});});}
export async function commitFiles(root:string,writes:{path:string;bytes:Buffer}[],write=atomic){return withWorkspaceWriteLock(root,async()=>{
