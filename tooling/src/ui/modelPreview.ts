...
export async function modelPreview(host:HTMLElement,url:string):Promise<()=>void>{
 const renderer=new WebGLRenderer({antialias:true,alpha:false});renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.setClearColor(new Color('#101c14'));host.replaceChildren(renderer.domElement);
