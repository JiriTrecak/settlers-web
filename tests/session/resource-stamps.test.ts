...
it('reuses unchanged observations but removes falling trees and refreshes same-tick view changes',()=>{
 ...
 const session=Object.assign(Object.create(Session.prototype),{loadedMap:{map:{stamps:[]}},mini:{setStamps:vi.fn()},resourceSignature:''});
 ...
 expect(session.stamps).toEqual([]);
 // Restored/authored map stamps invalidate the signature independently of trees.
 session.loadedMap.map.stamps=[{id:'new-map-prop'}];session.resourceSignature='';
