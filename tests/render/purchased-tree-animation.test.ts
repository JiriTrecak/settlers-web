...
for(const id of ['tree_primary','tree_secondary'])it(`${id} plays the game's felling sequence without shrinking`,async()=>{
 ...
 // Node has no image decoder. Keep actual meshes, rig and animation bytes;
 // substitute only texture decoding (the live browser validates appearance).
 const loader=new GLTFLoader().register(()=>({name:'geometry-test-textures',loadTexture:()=>Promise.resolve(new Texture())}));
 const gltf=await loader.parseAsync(bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength),'');
 for(const [name,duration] of [['hit',.6],['fall',1.8],['decay',6]] as const)expect(gltf.animations.find(a=>a.name===name)!.duration).toBeCloseTo(duration,5);
