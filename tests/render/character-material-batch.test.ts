...
for(const variant of ['base','warrior','archer','hunter','bombardier','marshal'] as const) it(`batches ${variant} without changing skinned vertices, PBR factors or ownership`,async()=>{
 ...
 let draws=0;after.root.traverse(o=>{if(o instanceof SkinnedMesh){draws++;const m=o.material as MeshStandardMaterial;if(m.name==='Character PBR palette'){expect(m.roughnessMap).toBe(m.metalnessMap);expect(m.roughness).toBe(1);expect(m.metalness).toBe(1);}}});
