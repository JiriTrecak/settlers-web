...
for m in leaves:
 ...
 tex=nodes.new('ShaderNodeTexNoise');tex.inputs['Scale'].default_value=5.0;tex.inputs['Detail'].default_value=1.2
 ramp=nodes.new('ShaderNodeValToRGB'); ramp.color_ramp.elements[0].position=.22;ramp.color_ramp.elements[0].color=tuple(c*.45 for c in color[:3])+(1,)
 ramp.color_ramp.elements[1].position=.8;ramp.color_ramp.elements[1].color=tuple(c*1.32 for c in color[:3])+(1,)
...
def tube(n,points,radii,sides=6):
 v=[]; f=[]
...
tube('Thick tapered brown stump',[(0,0,0),(.02,0,.22),(-.02,.02,.85),(.04,.03,1.65)],[.53,.46,.35,.27],sides=8)
for i in range(6):
...
def canopy(level, top, radius, depth):
 n=42; rings=2; verts=[]; faces=[]; phase=level*2.399
 def outline(a):
  broad=.5+.5*math.cos(7*a+phase)
  fingers=.5+.5*math.cos(14*a+phase+.45*math.sin(7*a))
  rad=1+.13*math.cos(7*a+phase)+.065*math.cos(14*a+phase)+.04*math.sin(3*a+phase)
  drop=.36*broad+.19*fingers
  return rad,drop
 ...
 for j in range(1,rings+1):
  ...
  for i in range(n):
   ...
   z+=.17*math.cos(7*a+phase)*math.sin(math.pi*t)+.035*math.sin(14*a+phase)*math.sin(math.pi*t)
 ...
 for k,(factor,dz) in enumerate([(.85,-.065)]):
  for i in range(n):
