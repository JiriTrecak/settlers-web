"""Great Mound: upgraded Rootbound Hall with a raised armored command storey.
...
scene.name = 'Great Mound · Ant tier two'
...
CLOTH = mat('TC_TeamColor', '#A04B31', 1, .92)
...
def root(name, controls, radii, material, lobes=12):
    ...
    rr = [max(.012,v[0]) for v in catmull([(r,0,0) for r in radii], 4)]
    verts=[]
...
# Tier-two command storey: broad eight-sided timber drum above the original lower eave.
# The outer foundation and entrance remain exactly where the Mound has them.
CURRENT=COLS['02 · Timber frame']
for i in range(8):
    a=(i+.5)*math.tau/8
    b=(i+1.5)*math.tau/8
    x,y=2.04*math.cos(a),1.45*math.sin(a)
    xx,yy=2.04*math.cos(b),1.45*math.sin(b)
    center=Vector(((x+xx)/2,(y+yy)/2,3.01))
    tangent=Vector((xx-x,yy-y,0))
    panel=box('Command storey hewn wall',center,(tangent.length+.04,.19,.98),WOOD[i%3],.025)
    panel.rotation_euler.z=math.atan2(tangent.y,tangent.x)
    # Horizontal dark recesses read as lookout slits rather than decorative microdetail.
    outward=Vector((center.x,center.y,0)).normalized()
    slit=box('Recessed lookout slot',center+outward*.12+Vector((0,0,.11)),(tangent.length*.53,.045,.17),DARK,.01)
    slit.rotation_euler.z=panel.rotation_euler.z
    for z in [2.59,3.45]:
        cone('Command storey iron belt',(x,y,z),(xx,yy,z),.095,.095,IRON,6)
    cone('Great Mound corner timber',(x,y,2.53),(x,y,3.5),.13,.13,WOOD[2],8)
    for z in [2.68,3.31]:
        cone('Command post iron collar',(x,y,z),(x,y,z+.14),.15,.15,IRON,8)
        p=Vector((x,y,z+.07));rivet(p+Vector((x,y,0)).normalized()*.15,(x,y,0),.055)

CURRENT=COLS['03 · Roof planks']
RX,RY,RZ=2.05,1.45,CONFIG['shape']['roof_height']
ZBASE=3.48
...
for xnorm in [-.78,-.3,.27,.77]:
    ...
    ribbon('Roof steel arch',path,.25 if abs(xnorm)>.6 else .31,IRON,dome_normal,True)
    for i in [12,31,48]:
...
ribbon('Upper transverse roof hoop',path,.22,IRON,dome_normal,True)
...
banner(0,0,6.65,1.40,1.75,pole_side=0,base_z=ZBASE+RZ-.06)
...
scene['source_note']='Tier-two evolution of the supplied Mound reference. Raised command storey and hidden surfaces are inferred; footprint unchanged.'
