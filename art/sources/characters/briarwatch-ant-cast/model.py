...
scene.render.fps=24
collections={n:bpy.data.collections.new(n) for n in ['Base anatomy','Warrior equipment','Archer equipment','Civilian clothing','Captain equipment','Rig','Studio']}
...
def material(name,color,metal=0,rough=.6):
    ...
    return m
TEAM=material('TC_TeamColor','#9a3f25',.05,.50) # Reference highlight interpreted as warm red shell albedo.
...
STEEL=material('Blue charcoal armor','#3e3931',.48,.44)
EDGE=material('Armor edge steel','#736a50',.48,.36)
BRONZE=material('Antique bronze trim','#997344',.45,.46)
...
def mesh(name,verts,faces,mat,bone,smooth=False):
    ...
    ob=bpy.data.objects.new(name,data);collections[{'base':'Base anatomy','warrior':'Warrior equipment','archer':'Archer equipment','civilian':'Civilian clothing','captain':'Captain equipment'}[role]].objects.link(ob)
...
# Briarwatch-specific costumes and weapons, on the proven ant locomotion rig.
LINEN=material('Undyed village linen','linen',0,.95)
LEAF=material('Leaf apron','leaf',0,.9)
OCHRE=material('Captain weathered ochre cloak','ochre',0,.95)
STONE=material('Chipped flint axe','flint',0,.9)
# Replace the Vanguard sword/shield silhouettes with a scavenged asymmetric axe.
for ob in list(parts):
    if ob.name.startswith(('Sword ','Shield ')):
        parts.remove(ob);bpy.data.objects.remove(ob,do_unlink=True)
role='warrior'
tube('Briar axe crooked haft',[(-.43,-.02,.79),(-.43,-.05,.59),(-.43,-.23,.20)],[.035,.030,.025],WOOD,'socket_hand.R',7)
mesh('Chipped stone axe head',[(-.48,-.16,.37),(-.48,-.49,.36),(-.48,-.57,.22),(-.48,-.49,.11),(-.48,-.20,.16),(-.39,-.16,.37),(-.39,-.49,.36),(-.39,-.57,.22),(-.39,-.49,.11),(-.39,-.20,.16)],[(0,1,2,3,4),(9,8,7,6,5),(0,5,6,1),(1,6,7,2),(2,7,8,3),(3,8,9,4),(4,9,5,0)],STONE,'socket_hand.R')
for z in [.25,.29,.33]:tube('Axe lashings',[(-.49,-.22,z),(-.43,-.28,z),(-.38,-.22,z),(-.43,-.16,z),(-.49,-.22,z)],[.014]*5,LEATHER,'socket_hand.R',5)
# Four lower auxiliary arms emphasize insect anatomy without interfering with the weapon hands.
role='base'
for sign,side in [(1,'L'),(-1,'R')]:
    tube('Lower arm '+side,[(sign*.19,.01,.88),(sign*.28,-.05,.75),(sign*.20,-.17,.67)],[.047,.036,.025],SHELL,'spine',7)
    ell('Small lower claw '+side,(sign*.20,-.17,.67),(.044,.025,.027),SHELL,'spine',8,4)
role='civilian'
# V-neck vest sits outside the thorax, with a split leaf apron for the walking legs.
for sign in [-1,1]:
    mesh('Open linen vest',[(sign*.02,-.172,1.11),(sign*.19,-.145,1.18),(sign*.245,-.135,.91),(sign*.17,-.17,.76),(sign*.025,-.188,.8)],[(0,1,2,3,4)],LINEN,'spine')
    mesh('Split apron',[(sign*.02,-.18,.77),(sign*.17,-.15,.77),(sign*.19,-.17,.47),(sign*.055,-.22,.43)],[(0,1,2,3)],LEAF,'hips')
    tube('Linen shoulder strap',[(sign*.18,-.13,1.17),(sign*.19,.01,1.22),(sign*.19,.14,1.09)],[.045,.042,.042],LINEN,'spine',6)
for i in range(3):tube('Woven waist cord',[(-.19,-.11,.78+i*.016),(0,-.20,.78+i*.016),(.19,-.11,.78+i*.016)],[.011]*3,WOOD,'hips',5)
ell('Leather seed pouch',(.22,-.04,.64),(.085,.064,.11),LEATHER,'hips',8,5)
role='archer'
# Ranger's leaf hood and cloth scarf, not the polished Vanguard armor.
mesh('Poacher mantle',[(-.27,.02,1.18),(0,.21,1.25),(.27,.02,1.18),(.30,.12,.96),(0,.25,.88),(-.30,.12,.96)],[(0,1,5),(1,4,5),(1,2,3,4)],LEAF,'spine')
role='captain'
for source in list(parts):
    if source['role']!='warrior':continue
    ob=source.copy();ob.data=source.data.copy();collections['Captain equipment'].objects.link(ob);ob.name='Captain '+source.name;ob['role']='captain';parts.append(ob)
for ob in list(parts):
    if ob['role']=='captain' and any(n in ob.name for n in ['Briar axe','Chipped stone','Axe lashings']):
        parts.remove(ob);bpy.data.objects.remove(ob,do_unlink=True)
tube('Captain heavy mace haft',[(-.43,-.015,.8),(-.43,-.06,.58),(-.43,-.22,.22)],[.041,.037,.035],WOOD,'socket_hand.R',8)
ell('Captain mace head',(-.43,-.24,.25),(.12,.15,.14),STEEL,'socket_hand.R',8,5)
for i in range(6):
    a=i*math.tau/6
    tube('Captain mace tooth',[(-.43+math.cos(a)*.1,-.24+math.sin(a)*.12,.25),(-.43+math.cos(a)*.20,-.24+math.sin(a)*.22,.25)],[.04,.001],BRONZE,'socket_hand.R',6)
# Broad segmented mantle and a heavy trailing cloak make the leader identifiable from above.
for sign,side in [(1,'L'),(-1,'R')]:
...
scene['source_reference']='Briarwatch cast-and-village concept sheet; original clothing and raider equipment, shared Vanguard anatomy/animation rig.'
