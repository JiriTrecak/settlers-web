...
const published=JSON.parse(readFileSync('assets/manifest.json'));
const destination='art/sources/environment/coniferous-pack/derived';mkdirSync(destination,{recursive:true});
for(const name of ['grass_v5_03','grass_v5_06']){
 const bytes=readFileSync(published.records.find(r=>r.scenery.some(s=>s.id===name)).outputs[0].path);
 ...
 for(const [level,retained] of [['medium',.38],['far',.14]]){
  ...
  writeFileSync(`${destination}/${name}-${level}.json`,JSON.stringify(mesh.toJSON()));
