function on_start()
 ...
 mission.say('Marshal','unit.ants.marshal','No wind. No sky. Only the tree around us. Keep to the amber lights. We find what poisons these roots, then we find our way home.',10,true)
end
function on_tick()
 ...
 if stage=='gallery' and mission.in_region('marshal','gallery') then
  ...
  mission.say('Vanguard scout','unit.ants.archer','The root rises above the gallery. Archers can take its crown while the shields cover the lower passage. I see amber burning in a chamber to the east.',10,true)
 elseif stage=='resin' and not mission.alive('resin-guard.0') and not mission.alive('resin-guard.1') then
  ...
  mission.say('Marshal','unit.ants.marshal','The living resin restores our strength. Tend the wounded while we can. Something has nested around its source. Take the northern passage. No one goes alone.',10,true)
 elseif stage=='heart' and not mission.alive('heart-keeper.0') and not mission.alive('heart-keeper.1') and not mission.alive('heart-keeper.2') then
  ...
  mission.say('Marshal','unit.ants.marshal','The Keeper is fallen. This is more than a mine, more than another resource to claim. The whole forest drinks from these roots. We must tell the colony.',11,true)
