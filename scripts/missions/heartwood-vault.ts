...
const landscape=emptyLandscape();landscape.environment={interior:true,floorMaterial:'heartwood',preset:'heartwood-interior',hour:10,season:'summer',playing:false,weather:{kind:'clear',intensity:0,windX:0,windZ:0},atmosphere:{...DEFAULT_ATMOSPHERE,enabled:true,color:'#8a8e9f',sunTint:'#e6cfab',density:.0015,baseHeight:0,heightFalloff:6,sunStrength:.3,noiseStrength:.3,driftSpeed:.12,regions:[{id:'underground-stream',x:132,y:1,z:155,radiusX:24,radiusY:2,radiusZ:9,density:.007}]}};
...
writeFileSync(new URL('../../assets/maps/campaign/vanguard-heartwood-vault.utcmap',import.meta.url),stringifyUtcMap(map));console.log({rooms:rooms.length,props:stamps.length,units:entities.length});
