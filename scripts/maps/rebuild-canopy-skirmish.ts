...
for(const [id,base] of [['worldroot-hollow',hollow],['four-crowns',fourCrowns()],['terrain-proving-ground',terrainLab()]] as const){
 ...
 writeFileSync(`assets/maps/skirmish/${id}.utcmap`,stringifyUtcMap(forestWarfareDressing(map)));console.log(`${id}: ${map.entities.length} entities, ${map.stamps.filter(s=>s.asset==='ancient-canopy-trunk').length} giant trunks, ${map.stamps.length} props`);
}
